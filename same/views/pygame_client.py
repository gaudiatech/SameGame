...
class PyGameClient(GuiClient):
    ...
    def draw_circle(self, position: tuple, colour: 'Colour'):
        ...
        pygame.draw.circle(self.screen, colour, (int(x * self.size), int(y * self.size)), self.size//2 - 2, 0)
        pygame.draw.circle(self.screen, colour, (int(x * self.size), int(y * self.size)), self.size//2 - 2, 1)
    ...
    def draw_score_board(self, score: int, highest_score: int, current_move_score: int, moves: int):
        ...
        myfont = pygame.font.Font(None, SCORE_FONT_SIZE)
